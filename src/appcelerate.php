...
class BPME {
	...
	private function getProcessCodeFromProcessInstance($id_process_instance) {
		...
		$sql="select processes.code from processes join process_instances on processes.id=process_instance.id_process where process_instances.id=$id_process_instance";
		$rs=$this->db->query($sql);
		try {
			$this->rsCheck($rs);
		}
	...
	private function getProcessNameFromProcessInstance($id_process_instance) {
		...
		$sql="select processes.name from processes join process_instances on processes.id=process_instance.id_process where process_instances.id=$id_process_instance";
		$rs=$this->db->query($sql);
		try {
			$this->rsCheck($rs);
		}
